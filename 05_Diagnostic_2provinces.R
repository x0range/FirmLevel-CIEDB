...
names(df)
  ...
  df_cut <- df %>% 
    ...
    filter(Province %in% c("Guangdong", "Zhejiang")) %>% # Provinces index
    filter(Year == 2007) %>% # Year index
...
fun_plot_marginal <- function(pdf_name, title, cond_name, var_name, x_lab, c_names, neg_cut, pov_cut, cut_num, plot_name){
  ...
  if (nrow(dd)>cut_num) {
    ...
    c_ind <-  which(c_names%in%c_uni)
        ...
        legend("topright", legend = c_names[c_ind], pch = q_25(c_ind), col = color_ind[c_ind], bty='n', xpd=NA, cex = .8, ncol = 1)
...
fun_plot_marginal(pdf_name = "Figure_notitle_2Provinces_2007_LP", title = "", cond_name = "Province", var_name = "def_LP_IO", x_lab = "LP", c_names = pro_name,  neg_cut = neg_cut, pov_cut = pov_cut, cut_num = 5000)

fun_plot_marginal(pdf_name = "Figure_notitle_2Provinces_2007_LP_Diff", title = "", cond_name = "Province", var_name = "def_LP_IO_diff", x_lab = "LP Change", c_names = pro_name,  neg_cut = neg_cut, pov_cut = pov_cut, cut_num = 5000)

fun_plot_marginal(pdf_name = "Figure_notitle_2Provinces_2007_RoFIAS", title = "", cond_name = "Province", var_name = "def_RoC_G_FI", x_lab = "Profitability (RoFIAS)", c_names = pro_name,  neg_cut = neg_cut, pov_cut = pov_cut, cut_num = 5000)
...
fun_plot_marginal(pdf_name = "Figure_2Provinces_2007_LP_Diff", title = "Log Density of Labor Productivty Change by Province", cond_name = "Province", var_name = "def_LP_IO_diff", x_lab = "LP Change", c_names = pro_name,  neg_cut = neg_cut, pov_cut = pov_cut, cut_num = 5000)
...
fun_plot_marginal(pdf_name = "Figure_2Provinces_2007_RoFIAS", title = "Log Density of Profitability (RoFIAS) by Province", cond_name = "Province", var_name = "def_RoC_G_FI", x_lab = "Profitability (RoFIAS)", c_names = pro_name,  neg_cut = neg_cut, pov_cut = pov_cut, cut_num = 5000)

fun_plot_marginal(pdf_name = "Figure_2Provinces_2007_IR", title = "Log Density of the Investment Rate by Province", cond_name = "Province", var_name = "def_FIAS_g", x_lab = "Investment Rate", c_names = pro_name,  neg_cut = neg_cut, pov_cut = pov_cut, cut_num = 5000)
